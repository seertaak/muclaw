...
auto Database::open(std::string_view path) -> void {
  ...
  if (!s.ok()) {
    ...
    throw std::runtime_error(
        fmt::format("Failed to open db: {}", s.ToString()));
  }
...
auto Database::put(std::string_view k, std::string_view v) -> void {
  if (!db)
    return;
  rocksdb::Status s =
      db->Put(rocksdb::WriteOptions{}, rocksdb::Slice{k.data(), k.size()},
              rocksdb::Slice{v.data(), v.size()});
  if (!s.ok()) {
...
auto Database::value(std::string_view k) const -> std::optional<std::string> {
  ...
  std::string v{};
  rocksdb::Status s =
      db->Get(rocksdb::ReadOptions{}, rocksdb::Slice{k.data(), k.size()}, &v);
  if (s.IsNotFound()) {
    return std::nullopt;
  }
  if (!s.ok()) {
