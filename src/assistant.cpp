...
auto Assistant::handle_message(int64_t chat_id, std::string text) -> asio::awaitable<void> {
    ...
    auto reply =
        co_await llm_.chat_completion("You are muclaw, a helpful technical AI assistant built in C++23.", text);
...
auto Assistant::start() -> asio::awaitable<void> {
    ...
    co_await tg_.poll([this](int64_t chat_id, std::string text) { return handle_message(chat_id, std::move(text)); });
}
